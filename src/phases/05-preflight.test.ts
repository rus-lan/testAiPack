...
describe('phase 05 — preflight', () => {
  ...
  // cli/pipeline.ts's `resolvePackVisibilityConfirmed` reads ONLY a
  // `pack-visibility` check whose `details` starts with the literal
  // `pack-visibility [<variant>/<pack>]` as proof of confirmed visibility —
  // anything else (including a passing check) is "not confirmed", the
  // honest default. Mirrored here rather than importing that function, to
  // keep this suite about phase 05's own output contract.
  const NOT_A_CONFIRMED_VISIBILITY_DETAIL = (details: string | undefined): boolean =>
    !/^pack-visibility \[[^/\]]+\/[^\]]+\]/.test(details ?? '')

  // CLI-installed pack (e.g. graphify: `pipx install graphifyy`, no
  // skill/agent/command/plugin files in its repo — the skill file is
  // generated by its own CLI at install time). Phase 03 legitimately
  // records zero instructions for it; gate 4 must not reject that as a
  // delivery bug when the pack declares `setup`.
  it('pack-visibility: CLI-installed pack (zero instructions, setup+check) → pass, NOT confirmed; gate 6 still proves presence', async () => {
    const root = makeTempDir()
    const homes = await buildVariantHomes(root, ['a'])
    const input = buildInput(
      homes,
      {
        packs: [{ name: 'graphify', ref: 'github:o/graphify', setup: 'pipx install graphifyy', check: 'graphify --version' }],
        variants: [{ name: 'a', packs: ['graphify'] }],
        baseline: 'a',
        workspacePath: root,
        outputPath: path.join(root, 'out'),
      },
      {
        packInstall: packInstallOf([
          { pack: 'graphify', packPath: '/nowhere', detectedType: 'all', registeredIn: [], instructions: [] },
        ]),
      },
    )
    const result = await runP(preflight(input))
    ...
    const check = checkByVariant(result, 'pack-visibility', 'a')
    expect(check?.passed).toBe(true)
    expect(NOT_A_CONFIRMED_VISIBILITY_DETAIL(check?.details)).toBe(true)
    // gate 6 (pack-functional) independently ran the check and proved presence.
    expect(
      result.packChecks?.some((c) => c.pack === 'graphify' && c.variant === 'a' && c.exitCode === 0),
    ).toBe(true)
  })

  // The dangerous middle: no instructions, `setup` present, but no `check`
  // either — nothing in preflight proves this pack is actually present.
  // Decision: pass rather than fail the run, but still NOT confirmed — the
  // report must not claim a presence nothing verified.
  it('pack-visibility: CLI-installed pack with NO check either (zero instructions, setup only) → pass, NOT confirmed, unverified', async () => {
    const root = makeTempDir()
    const homes = await buildVariantHomes(root, ['a'])
    const input = buildInput(
      homes,
      {
        packs: [{ name: 'graphify', ref: 'github:o/graphify', setup: 'pipx install graphifyy' }],
        variants: [{ name: 'a', packs: ['graphify'] }],
        baseline: 'a',
        workspacePath: root,
        outputPath: path.join(root, 'out'),
    ...
    expect(check?.passed).toBe(true)
    expect(NOT_A_CONFIRMED_VISIBILITY_DETAIL(check?.details)).toBe(true)
    // no `check` command declared → gate 6 never touches this pack at all.
    expect(result.packChecks ?? []).toEqual([])
  })
  ...
  // Same CLI-installed exception as gate 4, on the foreign side: a pack
  // delivered only via `setup` has nothing gate 5 can check for a leak
  // either. `smoke` (the non-declaring variant) must not fail preflight for
  // that reason alone — gate 6's foreign-must-fail direction (below) is what
  // actually proves absence when the pack declares `check`.
  it('foreign-pack-absent: CLI-installed pack (zero instructions, setup+check) foreign on another variant → pass, not a vacuous pass; gate 6 still enforces both directions', async () => {
    const root = makeTempDir()
    const homes = await buildVariantHomes(root, ['a', 'smoke'])
    spawnMock.mockImplementation((opts: { cwd: string }) =>
      Effect.succeed({
        exitCode: opts.cwd === homes['a']![0] ? 0 : 127,
        stdout: '',
        stderr: '',
        durationMs: 2,
        timedOut: false,
    ...
    const input = buildInput(
      ...
      {
        packs: [{ name: 'graphify', ref: 'github:o/graphify', setup: 'pipx install graphifyy', check: 'graphify --version' }],
        variants: [{ name: 'a', packs: ['graphify'] }, { name: 'smoke', packs: [] }],
        baseline: 'a',
        workspacePath: root,
        outputPath: path.join(root, 'out'),
      },
    ...
    const check = checkByVariant(result, 'foreign-pack-absent', 'smoke')
    expect(check?.passed).toBe(true)
    expect(check?.details).toContain('absence not confirmed by gate 5')
    // gate 6 independently proved both directions: functional where declared, not where foreign.
    expect(
      result.packChecks?.some((c) => c.pack === 'graphify' && c.variant === 'a' && c.exitCode === 0),
    ).toBe(true)
    expect(
      result.packChecks?.some((c) => c.pack === 'graphify' && c.variant === 'smoke' && c.exitCode !== 0),
    ).toBe(true)
  })

  // The dangerous middle on the foreign side: no instructions, `setup`
  // present, no `check` — nothing proves absence either. Same decision as
  // gate 4: pass, but leave it unverified rather than silently clean.
  it('foreign-pack-absent: CLI-installed pack with NO check either (zero instructions, setup only) foreign on another variant → pass, absence unverified', async () => {
    const root = makeTempDir()
    const homes = await buildVariantHomes(root, ['a', 'smoke'])
    const input = buildInput(
      homes,
    ...
    const check = checkByVariant(result, 'foreign-pack-absent', 'smoke')
    expect(check?.passed).toBe(true)
    expect(check?.details).toContain('not verified by preflight at all')
    // no `check` command declared → gate 6 never touches this pack at all.
    expect(result.packChecks ?? []).toEqual([])
  })
