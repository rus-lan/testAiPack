...
- ✅ error redaction: ошибка с inline `mcp:name:{"env":{"KEY":"secret"}}` в
  ...
  сообщении/контексте.
- ❌ НЕ покрыто (ticket): Stage 2 — несколько паков на один вариант
  одновременно (снятие Stage 1 guard'а фазы 00 не требует изменений здесь —
  доставка уже per-pack; см. `.research/n-way-variants/00-overview.md §2`).
