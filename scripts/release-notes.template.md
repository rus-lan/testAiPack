...
## v0.8.0 (report rendered in Russian, pack-setup contamination fix, pipx in docker image)

- **BREAKING for anything parsing `report.md`/`report.html` as text**: the generated report — `report.md`, `report.html`, `timeline.html`, `compare` output, the summary headline, the `report --rebuild` provenance section and its per-field notes, and the contamination detail text — is now rendered in Russian. Machine formats (`report.json`, `report.yaml`) are unaffected: field names stay English, `schemaVersion` does not change — only the VALUE of `summary.headlineResult` is Russian now. The CLI's own terminal output (run progress, `gc`/`doctor`/`list`, config validation errors) deliberately stays English.
- Packs installed entirely via `setup`, with no skill/agent/command/plugin files in the repo at all (e.g. graphify, whose `SKILL.md` is generated by its own CLI at install time, not checked into the repo) no longer fail preflight with "no registration instructions recorded". Preflight now passes such a pack without files to check, but marks visibility as NOT confirmed by gates 4/5 — presence is instead proven by gate 6 running the pack's `check` in every HOME. A pack with neither registration files nor `setup` is still a hard error.
- The docker image (`Dockerfile.opencode`) now installs `pipx` (and therefore Python) — packs are arbitrary repos with an arbitrary `setup` command, and PyPI-based packs (graphify's `graphifyy`, `code-review-graph`) used to fail with exit 127 under `--isolation docker` on `node:22-slim`, which ships no Python at all. Rebuild the image (`bash scripts/build-docker-image.sh`) to pick this up.
- **Fixed: a pack's `setup` used to contaminate only run-1 of its variant.** `setup` runs with cwd = run-1's app directory; anything it writes there is now copied into every other run's app directory of the same variant, excluded from the measured diff for all of them, and cross-checked afterward — a run left with a different set of setup-written paths than run-1 aborts the whole experiment. A `setup` that modifies a tracked file now aborts (`E_PACK_SETUP_FAILED`) instead of silently corrupting the measured diff.
- Fixed: under `--isolation docker`, a failed command reported `0ms` duration instead of its real elapsed time.
- Fixed: a copied skill was fingerprinted by its run-absolute install path, so every multi-run experiment with a skill pack falsely reported `install-drift` contamination between its own runs — now fingerprinted by content.
- Fixed: `report --rebuild` matched the English `## Summary` heading to splice in its provenance block — since the report is now Russian (`## Сводка`), the match silently failed and the block was prepended to the top of the file instead.

Full changelog: https://github.com/rus-lan/testAiPack/compare/v0.7.0...v0.8.0
